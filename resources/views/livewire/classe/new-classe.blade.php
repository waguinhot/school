<div x-data="{ modal: false }">
    <div class="mt-4 mb-4">
        <button
            @click="modal = true"
            class="flex justify-center w-full px-4 py-2 flex items-center text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md hover:bg-indigo-500 focus:outline-none focus:border-indigo-700 focus:ring-indigo active:bg-indigo-700 transition duration-150 ease-in-out">
            <x-icons.add /> <span class="ml-2">Nova Classe</span>
        </button>
    </div>

    <div x-show="modal"   id="authentication-modal" tabindex="-1" aria-hidden="true" class=" bg-gray-200 bg-opacity-50  flex h-screen justify-center items-center   overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50  w-full md:inset-0 h-[calc(100%-1rem)] max-h-full ">
        <div class="relative p-4 w-3/6 justify-center ">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 w-full " @click.away="modal = false">
                <!-- Modal header -->
